...
public class StoreSaleController {
    ...
    @PutMapping(value = "/takegood")
    @Log("进货")
    @ApiOperation("进货")
    @PreAuthorize("@el.check('storeRemain:edit')")
    public ResponseEntity<Object> takegood(@Validated @RequestBody StoreRemain resources){
        storeRemainService.takegood(resources);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }

    @PutMapping(value = "/refundgood")
    @Log("退货")
    @ApiOperation("退货")
    @PreAuthorize("@el.check('storeRemain:edit')")
    public ResponseEntity<Object> refundgood(@Validated @RequestBody StoreRemain resources){
        storeRemainService.refundgood(resources);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
