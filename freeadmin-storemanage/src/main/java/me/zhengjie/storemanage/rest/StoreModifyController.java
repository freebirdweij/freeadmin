...
@Api(tags = "库存管理")
@RequestMapping("/api/storeModify")
public class StoreModifyController {
    ...
    @PutMapping(value = "/movegood")
    @Log("调拨")
    @ApiOperation("调拨")
    @PreAuthorize("@el.check('storeRemain:edit')")
    public ResponseEntity<Object> movegood(@Validated @RequestBody StoreRemain resources){
        storeRemainService.movegood(resources);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }

    @PutMapping(value = "/modifygood")
    @Log("盘点")
    @ApiOperation("调拨")
    @PreAuthorize("@el.check('storeRemain:edit')")
    public ResponseEntity<Object> modifygood(@Validated @RequestBody StoreRemain resources){
        storeRemainService.update(resources);
        return new ResponseEntity<>(HttpStatus.NO_CONTENT);
    }
